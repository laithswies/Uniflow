...
def send_post_request():
    ...
    for key, value in vehicleType.items():
        url = "http://localhost:8080/api/visit-statistics"
        payload = {
            "visitDate": datetime.now().strftime("%Y-%m-%d"),
            "vehicleType": key,
            "count": side_settings['side1']['counts'][value]['up']+
                     side_settings['side1']['counts'][value]['down']+
                     side_settings['side2']['counts'][value]['up']+
                     side_settings['side2']['counts'][value]['down']
        }
...
def get_count():
    result = {
        'current_gas':side_settings['side1']['current_vehicles'],
        'current_diesel':side_settings['side2']['current_vehicles'] 
    }
    return result
...
def run_post_counts():
    schedule.every().day.at("23:58").do(send_post_request)
...
if __name__ == "__main__":
    ...
    post_thread = Thread(target=run_post_counts)
    post_thread.start()

    fastapi_thread = Thread(target=run_fastapi)
    fastapi_thread.start()

    rtsp_url = 'rtsp://.:cam@1234@127.0.0.1:37958/cam/realmonitor?channel=1&subtype=1'
